...
def get_unwind(full_node_client: FullNodeRpcClient, genesis_coin_id: bytes32, tail_hash_bytes: bytes32, parent_puzzle_lookup: Dict[str, TargetCoin], target_puzzle_hash: bytes32) -> List[CoinSpend]:
...
def unwind_coin_spend(full_node_client: FullNodeRpcClient, wallet_client: WalletRpcClient, tail_hash_bytes: bytes32, coin_spend: CoinSpend):
...
def cli(
    ...
    if unwind_target_puzzle_hash_bytes is not None:
        ...
        required_coin_spends: List[CoinSpend] = get_unwind(full_node_client, genesis_coin_id, tail_hash_bytes, parent_puzzle_lookup, current_puzzle_hash)
        ...
        for coin_spend in required_coin_spends[::-1]:
            unwind_coin_spend(full_node_client, wallet_client, tail_hash_bytes, coin_spend)
    else:
        ...
        for batch_targets in batched_targets:
            current_puzzle_hash = construct_cat_puzzle(CAT_MOD, tail_hash_bytes, batch_targets[0].puzzle_hash).get_tree_hash(batch_targets[0].puzzle_hash)
            required_coin_spends: List[CoinSpend] = get_unwind(full_node_client, genesis_coin_id, tail_hash_bytes, parent_puzzle_lookup, current_puzzle_hash)

            print(f"{len(required_coin_spends)} spends required to unwind the bag to {batch_targets[0].puzzle_hash}")
