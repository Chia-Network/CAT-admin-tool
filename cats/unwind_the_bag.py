...
async def app(
    ...
    full_node_client = await FullNodeRpcClient.create(
        chik_config["self_hostname"],
        chik_config["full_node"]["rpc_port"],
        chik_root,
        load_config(chik_root, "config.yaml"),
    )
    wallet_client = await WalletRpcClient.create(
        chik_config["self_hostname"],
        chik_config["wallet"]["rpc_port"],
        chik_root,
        load_config(chik_root, "config.yaml"),
    )
...
def cli(
    ...
    eve_coin_id_bytes = bytes32.fromhex(eve_coin_id)
    ...
    chik_root: Path = Path(
        os.path.expanduser(os.getenv("CHIK_ROOT", "~/.chik/mainnet"))
    ).resolve()
